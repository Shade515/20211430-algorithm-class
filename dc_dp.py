...
# ========================================
#   1. 배열(arr)의 합 문제 : 억지기법 vs. 분할정복

def sum_bf(arr): #시간복잡도 - 0(n), n = len(arr), 공간복잡도 - O(1)
    """억지기법 - 단순 반복문"""
    total = 0
    for x in arr:
        total += x
...
def power_dc(x,n):
    ...
    if n == 1: # 종료 조건
        ...
        return power_dc(x*x, (n-1)//2) # 재귀호출 - 절반크기로 감소 - O(logn)

#테스트 코드
x = 2.0
n = 10
print(f"억지기법_ x의 n 거듭제곱({x}, {n}) = {power_bf(x,n)} ")
print(f"출소정복_ x의 n 거듭제곱({x}, {n}) = {power_dc(x,n)} ")
print("="* 100)