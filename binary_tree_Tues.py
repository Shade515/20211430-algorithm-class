...
def is_leaf(node): # 노드가 None이 아니고, 왼쪽과 오른쪽 자식 노드가 모두 None인 경우 단말노드 (단말노드 정의)
	if node is None:
		return False
...
def count_nodes(root): # 이진트리의 총 노드의 개수
	# 재귀적 정의 : 트리가 비어 있으면 0, 아니면 왼족과 오른쪽 서브트리의 노드의 수 합산 + 1
...
def count_edges(root): # 이진 트리의 총 간선(엣지, 연결된 링크) 수 구하기
	...
	return max(0, nodes-1) # 노드가 0인 경우 음수가 되지 않도록 0반환
...
def full_binary_tree_nodes(k): # 높이가 K인 포화 이진 트리의 노드 개수 구하기 (루트레벨 +1)
	return 2 ** k - 1
...
if __name__ == "__main__":
	...
	print("높이 5인 이진트리의 최대 노드 노드 수:", max_nodes_in_binary_tree(5))
